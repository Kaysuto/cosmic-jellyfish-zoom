...
async function testAuthFix() {
  console.log('🔧 Test de la correction d\'authentification Jellyfin...\n');

  try {
    const username = 'Kimiya';
    const password = 'ENZlau2025+';
    
    console.log('📊 Authentification avec correction...');
    
    // Utiliser la nouvelle fonction de correction
    const { data: authData, error: authError } = await supabase.functions.invoke('jellyfin-auth-fix', {
      body: {
        username: username,
        password: password
      }
