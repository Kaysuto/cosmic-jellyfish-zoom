...
interface JellyfinAuthFixRequest {
  username: string;
  password: string;
}
...
serve(async (req) => {
  ...
  try {
    const { username, password }: JellyfinAuthFixRequest = await req.json().catch(() => ({}));
    ...
    const { data: profile, error: profileError } = await supabaseAdmin
      .from('profiles')
      .select('*')
      .eq('jellyfin_user_id', authData.User.Id)
      .single();
    ...
    // 4. Mettre à jour le mot de passe de l'utilisateur Supabase
